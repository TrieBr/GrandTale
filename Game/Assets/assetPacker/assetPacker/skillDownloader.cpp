...
#include "skillDownloader.h"
	...
	bool skillDownloader::connect(std::string host, std::string db, std::string username, std::string password) {
	...
	int skillDownloader::downloadSkills(std::string fileDestination) {
		...
		if (fOut->is_open()) {
			...
			if (query("SELECT id,skillName, skillType, iconID FROM skills")) {
	...
	bool skillDownloader::query(std::string queryString) {
