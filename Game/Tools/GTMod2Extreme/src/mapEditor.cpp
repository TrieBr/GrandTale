			...
			case ALLEGRO_EVENT_KEY_UP: {
						...
						if (ev.keyboard.keycode==ALLEGRO_KEY_B) {
							if (al_key_down(&kS,ALLEGRO_KEY_LCTRL)) {
								...
								this->mHandler->mData->backgroundLayers.push_back(nBG);
								nBG = new mapBackground();
								nBG->xPos = 0;
								nBG->yPos = 0;
								nBG->bgHandle = backgroundManager::loadBackground(2);
								this->mHandler->mData->backgroundLayers.push_back(nBG);
								 nBG = new mapBackground();
								nBG->xPos = 0;
								nBG->yPos = 0;
								nBG->bgHandle = backgroundManager::loadBackground(0);
								this->mHandler->mData->backgroundLayers.push_back(nBG);
								nBG = new mapBackground();
								nBG->xPos = 0;
								nBG->yPos = 0;
								nBG->bgHandle = backgroundManager::loadBackground(1);
								this->mHandler->mData->backgroundLayers.push_back(nBG);
...
void mapEditor::saveMap() {
	ALLEGRO_FILECHOOSER* fC = al_create_native_file_dialog("","Select Location..","*.map",ALLEGRO_FILECHOOSER_SAVE);
